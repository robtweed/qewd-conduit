...
function create(authorId, data) {
  ...
  var articlesDoc = this.db.use('conduitArticles');
...
function getTags() {
  var tags = [];
  var tagsDoc = this.db.use('conduitArticles', 'byTag');
...
function getIdBySlug(slug) {
  var slugIndex = this.db.use('conduitArticles', 'bySlug', slug);
  if (!slugIndex.exists) {
    return false;
  }
...
function get(articleId, byUserId) {
  var articlesDoc = this.db.use('conduitArticles', 'byId', articleId);
...
function favoritedBy(username, byUserId, offset, max) {
  ...
  var articlesDoc = this.db.use('conduitArticles');
  var favDoc = this.db.use('conduitUsers', 'byId', userId, 'favorited');
...
function latest(byUserId, offset, max) {
  ...
  articlesDoc.$('byTimestamp').forEachChild(function(ts, childNode) {
    if (offset > 0 && skipped < offset) {
      ...
      var article = get.call(self, articleId, byUserId);
      articles.push(article);
      count++;
      if (count === max) return true;
    }
