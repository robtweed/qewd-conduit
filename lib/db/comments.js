...
function create(authorId, articleId, commentBody) {
  ...
  var commentsDoc = this.db.use('conduitComments');
...
function del(commentId, unlinkArticle) {
  ...
  var commentDoc = this.db.use('conduitComments', 'byId', commentId);
...
function byUser(userId, articleId) {
 ...
 var comments = [];
  ...
  var commentsDoc = this.db.use('conduitArticles', 'byId', articleId, 'comments');
  var self = this;
  commentsDoc.forEachChild(function(commentId) {
    var comment = get.call(self, commentId, userId);
