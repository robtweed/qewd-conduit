...
function exists(id) {
  return this.db.use('conduitUsers', 'byId', id).exists;
}

function emailExists(email) {
  return this.db.use('conduitUsers', 'byEmail', email).exists;
}

function getEmail(id) {
  return this.db.use('conduitUsers', 'byId', id, 'email').value;
}

function changeEmail(id, newEmail) {
  let oldEmail = getEmail.call(this, id);
  let usersDoc = this.db.use('conduitUsers');
  let emailIndex = usersDoc.$('byEmail');
...
function usernameExists(username) {
  return this.db.use('conduitUsers', 'byUsername', username).exists;
}

function idByUsername(username) {
  return this.db.use('conduitUsers', 'byUsername', username).value;
}

function changeUsername(id, newUsername) {
  let usersDoc = this.db.use('conduitUsers');
  let usernameIndex = usersDoc.$('byUsername');
  let usernameNode = usersDoc.$(['byId', id, 'username']);
...
function getUsername(id) {
  return this.db.use('conduitUsers', 'byId', id, 'username').value;
}

function idByEmail(email) {
  if (emailExists.call(this, email)) {
    return this.db.use('conduitUsers', 'byEmail', email).value;
  }
  else {
    return false;
  }
...
function authenticate(email, password) {
  if (emailExists.call(this, email)) {
    let id = idByEmail.call(this, email);
    let hash = this.db.use('conduitUsers', 'byId', id, 'password').value;
...
function getNextId() {
  return this.db.use('conduitUsers', 'nextId').increment();
}
...
function favorited(userId, articleId) {
  return this.db.use('conduitUsers', 'byId', userId, 'favorited', articleId).exists;
}

function favorite(userId, articleId) {
  let favoritedDoc = this.db.use('conduitUsers', 'byId', userId, 'favorited', articleId);
  let favoriteCountDoc = this.db.use('conduitArticles', 'byId', articleId, 'favoritesCount');
...
function getProfile(ofUserId, byUserId) {
  ...
  let usersDoc = this.db.use('conduitUsers', 'byId');
  let ofUserDoc = usersDoc.$(ofUserId);
  if (!ofUserDoc.exists) {
...
function follows(userId, usernameToFollow) {
  let idToFollow = idByUsername.call(this, usernameToFollow);
  return this.db.use('conduitUsers', 'byId', userId, 'follows', idToFollow).exists;
}

function follow(userId, usernameToFollow) {
  var idToFollow = idByUsername.call(this, usernameToFollow);
  this.db.use('conduitUsers', 'byId', userId, 'follows', idToFollow).value = idToFollow;
...
function unfollow(userId, usernameToUnfollow) {
  let idToUnfollow = idByUsername.call(this, usernameToUnfollow);
  this.db.use('conduitUsers', 'byId', userId, 'follows', idToUnfollow).delete();
